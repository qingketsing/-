...
如果不使用异步化，所有的请求会直接的打到业务逻辑上，直接进行runtime相关的工作，等待回复http才会返回。但是LLM可能会持续十几秒，HTTP容易超时；请求断开会影响任务执行，而且服务重启后，进行中的请求难以恢复。

使用MQ之后，可以快速返回HTTP 202 Accept，然后让agent在后台运行，可以实现削峰填谷，同时结合message job和outbox可以实现失败重试，可靠的信息发布，以及独立的扩容。
